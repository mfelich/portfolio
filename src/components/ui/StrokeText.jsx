...
const StrokeText = ({
  ...
  const [measuredFontSize, setMeasuredFontSize] = useState(fontSize);
  ...
  const dash = Math.max(measuredFontSize * 7, 200);
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      const fs = parseFloat(window.getComputedStyle(rootRef.current).fontSize);
      if (fs > 0) {
        setMeasuredFontSize((prev) => (Math.abs(prev - fs) < 0.5 ? prev : fs));
      }
    };
  ...
  }, [fontsReady, characters, fontSize, fontWeight, letterSpacing, strokeWidth]);

  useLayoutEffect(() => {
    const root = rootRef.current;
    if (typeof window === "undefined" || !root || !box || !fontsReady) {
      return undefined;
    }
  ...
  const ready = fontsReady && box;

  const viewBox = box
    ? `${box.x} ${box.y} ${box.width} ${box.height}`
    : `0 ${-measuredFontSize} 600 ${measuredFontSize * 1.3}`;
  ...
  return (
    ...
    >
      <svg
        className="block"
        style={{ height: `${Math.round(measuredFontSize * 1.3)}px` }}
        viewBox={viewBox}
